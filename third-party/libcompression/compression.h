...
#else
/*  Stubs for the Apple-only headers.  Outside of Apple platforms we provide
    our own implementation of libcompression (third-party/libcompression/),
    so the availability annotations are simply elided.                       */
...
#if !defined(__clang__) && !defined(__GNUC__) && !defined(__restrict)
#  if defined(_MSC_VER)
